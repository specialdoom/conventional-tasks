  ...
  const items = [
    {
      id: "item-1",
      title: "Completed",
      description: "Completed tasks here"
    }
  ];
</script>

<div {...$root}>
  {#each items as { id, title, description } (id)}
    <div
      {...$item(id)}
      use:item
    >
...
<style>
  .cui--accordion-trigger {
    display: flex;
    width: 100%;
    padding: var(--size-spacing-1, 8px) var(--size-spacing-2, 16px);
    flex-direction: row;
    align-items: center;
  ...
  .cui--accordion-trigger svg {
    transition: all 0.11s cubic-bezier(0.2, 0, 0.38, 0.9);
    transform: rotate(0deg);
  }

  .cui--accordion-trigger.expanded svg {
  ...
  .cui--accordion-content {
    display: flex;
    padding: var(--size-spacing-1, 8px) var(--size-spacing-2, 16px)
      var(--size-spacing-2, 16px) var(--size-spacing-2, 16px);
